...
class InterFormerEncoder(nn.Module):
    def __init__(self,tag_vocab, num_layers, hidden_size, n_head, max_seq_len, max_lex_num,feedforward_dim, softmax_axis=-1, four_pos_fusion= 'ff',
        ...
        for i in range(self.num_layers):
            setattr(self, 'layer_{}'.format(i), InterFormerLayer(tag_vocab,hidden_size, n_head, max_lex_num,feedforward_dim, softmax_axis=softmax_axis,
    ...
    def forward(self, chars, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num, char_ids=None, word_ids=None):
        ...
        for i in range(self.num_layers):
            ...
            output = now_layer(output, words, pos_s, pos_e, lex_s, lex_e,seq_len, lex_num, rel_pos_embedding, char_ids, word_ids)

        return output


class InterFormerLayer(nn.Module):
    def __init__(self,tag_vocab, hidden_size, n_head, max_lex_num,feedforward_dim, softmax_axis=-1, fc_dropout=0.3, attn_dropout=0,
                 q_proj=True, k_proj=True, v_proj=True, r_proj=True, scale=False, vocab=None):
        super().__init__()

        self.attn = InterAttention(tag_vocab,hidden_size, n_head,max_lex_num, attn_dropout=attn_dropout, softmax_axis=softmax_axis,
    ...
    def forward(self, chars, words,pos_s, pos_e, lex_s, lex_e, seq_len, lex_num, rel_pos_embedding, char_ids, word_ids):
        #residual = chars

        output = self.attn(chars, words,pos_s, pos_e, lex_s, lex_e, seq_len, lex_num, rel_pos_embedding, char_ids, word_ids)
...
class InterAttention(nn.Module):
    def __init__(self,tag_vocab, hidden_size, n_head, max_lex_num,attn_dropout=0, softmax_axis=-1,
                 q_proj=True, k_proj=True, v_proj=True, r_proj=True, scale=False, vocab=None):
        super().__init__()
        ...
        self.n1=nn.Linear(self.n_head, 1)
        self.dropout = nn.Dropout(attn_dropout)
        self.wlt=nn.Linear(max_lex_num, len(tag_vocab))
        self.max_lex_num=max_lex_num
        self.tag_length=len(tag_vocab)
    ...
    def forward(self, chars, words, pos_s, pos_e, lex_s, lex_e,seq_len, lex_num, rel_pos_embedding, char_ids, word_ids):
        ...
        mask_s=pos_ss.eq(0).unsqueeze(1)
        mask_e=pos_ee.eq(0).unsqueeze(1)
        class_b=att_score*mask_s
        class_e=att_score*mask_e
        class_ms=att_score-class_b-class_e
        class_s=class_ms[:,:,:,0]
        class_m=class_ms[:,:,:,1:]
        
        class_b=torch.sum(class_b,-1,keepdim=True)
        class_e=torch.sum(class_e,-1,keepdim=True)
        class_m=torch.sum(class_m,-1,keepdim=True)
        class_score=torch.cat([class_b,class_e,class_s.unsqueeze(-1),class_m],-1)
        class_score=F.pad(class_score,[2,0])
        
        tag_score=self.n1(class_score.permute(0,2,3,1).contiguous()).squeeze(-1)
        tag_score = F.softmax(tag_score, dim=self.softmax_axis)
        if self.training:
            self.times += 1
        # if self.times == 1:
        #     print(att.size())
        if self.times % 13660 == 0 or self.times ==1366:
            self.count+=1
            # print(att[1,1,:])
            # print(att.size())
            # print(att[1])
            ...
            tag_data = tag_score.cpu().detach().numpy()[index_][:seq_length, :tag_length].T
            np.savetxt("./plt/tagfile.log", tag_data)
            plt.figure(figsize=(seq_length // 4 + 2, tag_length // 4 + 2))
            plt.rcParams['font.sans-serif'] = ['SIMHEI']  # 设置字体为黑体
            row_chars = char_ids[index_][:seq_length].cpu().numpy().tolist()
            row_tags = self.tag_vocab
            row_chars = [self.vocab.to_word(x) for x in row_chars]
            row_tags = [x for x in row_tags]
            sns.heatmap(tag_data,
                        xticklabels=row_chars,
                        yticklabels=row_tags,
                        cbar_kws={"orientation": "horizontal"})
            
            plt.savefig('./plt/btag{}.png'.format(self.count))
            plt.show()
            def attention_visualization(head_i,count):
                ...
                class_data = class_score.cpu().detach().numpy()[index_][head_i][:seq_length, :tag_length].T
                
                #fitlog.add_hyper_in_file(data)
                np.savetxt("./plt/ifile.log", data)
                np.savetxt("./plt/cfile.log", class_data)
                ...
                if not char_ids is None and not word_ids  is None:
                    ...
                    plt.savefig('./plt/bc{}a{}.png'.format(count,head_i))
                    plt.show()
            # output all head
        ...
        #value_weighted_sum = torch.matmul(att_score, char_value)
