...
from models.CLSEG import CLSEG
...
parser.add_argument('--dataset', type=str, default='weibo', choices=['weibo', 'resume', 'ontonotes', 'msra','pku','cityu','msr','as'])
...
max_lex_num = max(* map(lambda x:max(x['lex_num']),datasets.values()))
if use_bigram:
...
trainer = Trainer(datasets['train'], model, optimizer, batch_size=batch_size, sampler=SequentialSampler(),
                  ...
                  use_tqdm=True, print_every=5, save_path=None)
